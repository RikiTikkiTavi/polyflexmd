...
of the chain. The resulting MSD curves on different scales are shown on 
Figure \ref{fig:msd_anchored_zeta}. The exponential decay on time scales 
$t>\tau_{rot}$ is shown on Figure \ref{fig:msd_anchored_zeta-arm_fit} together
with fit of Adjusted Rouse model (Eq. \ref{eq:adjusted_rouse_model_ete})
and corresponding estimated values of free parameters. The $\frac{3}{4}$ scaling
regime on time scale $t \ll \tau_{rot}$ is shown on Figure 
\ref{fig:alpha_anchored_zeta}.
