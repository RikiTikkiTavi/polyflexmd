...
(however second one is outside the CI, 
which can be explained with abcent hydrodynamic and excluded volume interactions) to the ones
...
\\
Thirdly, it is observed, that an
increase of the $\zeta_e$ from the $\zeta_e=\zeta=1$ to the $\zeta_e=10$ 
...
\\
\\
Furthermore, analyzing transition $a \rightarrow d$ (\autoref{fig:alpha_fm_free}, $a \rightarrow d$), 
...
(which can be explained with abcent hydrodynamic and excluded volume interactions), to the $\alpha_{min}$
...
\\
\\
To summarize, we have shown, that the change of $\alpha_{min}=3/4$ to $\alpha_{min}=2/3$
observed by Singh \emph{et al.} \cite{Singh:2022} upon the binding of RAB5 to the EEA1
can be caused by both, 
stiffness transition of the chain and increase of friction coefficient of the chain end. 
...
The $\alpha_{min}(a)$ matches within the confidence interval
the $\alpha_{min}$ of EEA1 obtained by Singh \emph{et al.} \cite{Singh:2022}.
The $\alpha_{min}(c)$ does not match the $\alpha_{min}$ of EEA1+RAB5 obtained 
...
causes the decreases of the $\alpha_{min}$ (Eq. \ref{eq:alpha_min_zeta_e_trans}, \autoref{fig:alpha_fm_free}),
with an effect being smaller with decreasing stiffness. The difference, $\Delta \alpha_{min}(a \rightarrow b)$ 
...
to the one observed by Singh \emph{et al.} \cite{Singh:2022}, which is explained with abcence of hydrodynamic
and excluded volume interactions in the simulation. The difference, $\Delta \alpha_{min}(a \rightarrow d)$,
is in agreement, within the CI, to the one observed by Singh \emph{et al.} \cite{Singh:2022}.
Therefore, we have shown (taking into account the limitations of this work), 
that the change of $\alpha_{min} = 3/4$ to $\alpha_{min} = 2/3$ observed by
Singh \emph{et al.} \cite{Singh:2022} upon the binding of RAB5 to the EEA1 can be caused by both, stiffness
transition of the chain and increase of friction coefficient of the chain end.
...
theoretical requirement $l_b \ll L$ and the length of the EEA1 chain. Then, an improvement 
